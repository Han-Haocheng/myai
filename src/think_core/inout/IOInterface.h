...
public:
  ...
  {
    weight_t<active_t> emotionActive       = 1.0;//情绪活跃权重
    weight_t<link_val> mentalConcentration = 1.0;//精神集中权重
  };
...
inline bool IOInterface::saveConstId()
{
...
void IOInterface::initConstId(NodeIdAllocator &IdAlloc,
                              std::unordered_map<node_id, std::shared_ptr<NodeInfo>> &NodeBuffer)
{

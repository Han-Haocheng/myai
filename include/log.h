...
public:
  ...
  inline LogEvent(uint64_t timestamp, std::string file, int line, std::string logger_name, LogEvent::Level level, std::string thread_name, int thread_id, int fiber_id, std::string func_name);

  inline uint64_t getTimestamp() const;
  inline LogEvent::Level getLevel() const;
  ...
  inline int getFiberId() const;
  inline const std::string &getFuncName() const;
  inline std::stringstream &msg();
...
private:
  ...
  LogEvent::Level m_defLoggerLevel = LogEvent::Level::LL_UNKNOWN;

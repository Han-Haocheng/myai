...
uint64_t LogEvent::getTimestamp() const { return m_timestamp; }
LogEvent::Level LogEvent::getLevel() const { return m_level; }
const std::string &LogEvent::getFileName() const { return m_fileName; }
int LogEvent::getLine() const { return m_line; }
const std::string &LogEvent::getLoggerName() const { return m_loggerName; }
const std::string &LogEvent::getThreadName() const { return m_threadName; }
int LogEvent::getThreadId() const { return m_threadId; }
int LogEvent::getFiberId() const { return m_fiberId; }
const std::string &LogEvent::getFuncName() const { return m_funcName; }
std::stringstream &LogEvent::msg() { return m_msg; }
...
{
  if (m_level <= event->getLevel()) std::cout << m_formatter->format(event);
}
...
Logger::ptr LoggerManager::addLogger(const std::string &name, LogEvent::Level logger_level, LogFormatter::ptr formatter, const std::vector<LogAppender::ptr> &appenders)
{
  ...
  Logger::ptr logger{new Logger(name, LogEvent::Level::LL_DEBUG)};
  if (formatter)
    logger->setFormatter(std::move(formatter));
...
Logger::ptr LoggerManager::config_logger(const LoggerConfigVal &conf)
{
  ...
  auto level = conf.level == LogEvent::Level::LL_UNKNOWN ? m_defLoggerLevel : conf.level;
